<div class="flex items-center">
    <form action="/tweets/{{ $tweet->id }}/like" method="POST">
        ...
        <div class="flex items-center mt-3">
            <button type="submit">
                <svg viewBox="0 0 20 20" class="text-gray-500 mr-2 w-5 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-600' : '' }}">
            ...
            <span class="text-xl text-gray-500 mr-4">{{ $tweet->liked ?? 0 }}</span>
        </div>
    ...
    <form action="/tweets/{{ $tweet->id }}/like" method="POST">
        ...
        <div class="flex items-center mt-3">
            <button type="submit">
                <svg viewBox="0 0 20 20" class="text-gray-500 mr-2 w-5 {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-600' : '' }}">
                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g class="fill-current">
            ...
            <span class="text-xl text-gray-500">{{ $tweet->disliked ?? 0 }}</span>
        </div>
